...
VOICES={"tr":"tr-TR-EmelNeural","en":"en-US-AriaNeural","es":"es-ES-ElviraNeural","ar":"ar-SA-ZariyahNeural"}; LABELS={"tr":"TÜRKÇE","en":"ENGLISH","es":"ESPAÑOL","ar":"العربية"}
...
def frame(title,body,badge,seed,category,lang):
 im=bg(seed); d=ImageDraw.Draw(im,"RGBA"); rtl_mode=lang=="ar"; fbig=font(FA if rtl_mode else FB,82); fbody=font(FA if rtl_mode else FM,43)
 ...
 d.rounded_rectangle((PAD-10,370,W-PAD+10,1585),46,fill=(4,15,30,62),outline=(255,255,255,55),width=2); d.rounded_rectangle((PAD,390,W-PAD,510),35,fill=(255,255,255,24),outline=(255,255,255,70),width=2); d.rounded_rectangle((PAD+24,414,PAD+43,486),9,fill=ORANGE+(255,))
 label=rtl(LABELS[lang]) if rtl_mode else LABELS[lang]; d.text((PAD+65,425),f"{label}  •  {badge}",font=font(FA if rtl_mode else FB,27),fill=ORANGE)
 y=610
 for line in wrap(d,title,fbig,W-2*PAD-20):
  txt=rtl(line) if rtl_mode else line; lw=d.textlength(txt,font=fbig); d.text((W-PAD-lw if rtl_mode else PAD,y),txt,font=fbig,fill=WHITE); y+=98
 y+=35
 for line in wrap(d,body,fbody,W-2*PAD-20):
  txt=rtl(line) if rtl_mode else line; lw=d.textlength(txt,font=fbody); d.text((W-PAD-lw if rtl_mode else PAD,y),txt,font=fbody,fill=MUTED); y+=64
 d.rounded_rectangle((PAD,H-250,W-PAD,H-130),30,fill=(0,0,0,45),outline=(255,255,255,55),width=2); d.text((PAD+30,H-215),"BİL • MERAK ET • PAYLAŞ",font=font(FB,25),fill=WHITE)
...
def make_voice(path,fact,lang):
 x=fact[lang]; hook=fact.get("reel_hook",{}).get(lang) or x["headline"]; cta={"tr":"Böyle bilgiler için FactBite'ı takip et.","en":"Follow FactBite for more facts like this.","es":"Sigue a FactBite para descubrir más datos.","ar":"تابع FactBite لمزيد من المعلومات."}[lang]; subprocess.run(["edge-tts","--voice",VOICES[lang],"--text",f"{hook}. {x['body']} {cta}","--write-media",path],check=True,timeout=45)
def main():
 d=open("latest_post_dir.txt",encoding="utf-8").read().strip(); fact=json.load(open("fact.json",encoding="utf-8")); lang=fact.get("_language","tr"); x=fact[lang]; cat=fact.get("_category","general"); hook=fact.get("reel_hook",{}).get(lang) or x["headline"]; q=fact.get("story_question",{}).get(lang) or hook
 ...
 voice=os.path.join(d,"reel_voice.mp3"); music=os.path.join(d,"reel_music.wav"); make_voice(voice,fact,lang); make_audio(music); out=os.path.join(d,"factbite_reel.mp4"); cmd=["ffmpeg","-y"]
 for i in range(1,6): cmd += ["-loop","1","-t","4.6","-i",os.path.join(d,f"reel_{i}.png")]
 cmd += ["-i",music,"-i",voice]; f=[]
 ...
 cmd += ["-filter_complex",";".join(f),"-map","[vout]","-map","[aout]","-t","23","-c:v","libx264","-preset","veryfast","-crf","21","-pix_fmt","yuv420p","-c:a","aac","-b:a","160k","-movflags","+faststart",out]; subprocess.run(cmd,check=True); os.remove(music); os.path.exists(voice) and os.remove(voice)
